# To show what values can do, I'll run this ray tracer program. I'll turn on optimizations
# by passing `--release` to the compiler.
#
# ...
#
# OK, not bad, 2 seconds to render that. Now, if we go to the code we can see all types
# are defined as classes. For example Vec3, which represents a 3 dimensional vector, is
# a class. But lots of vectors are created to compute the ray tracing, and they are basically
# just wrappers for three numbers, plus they are immutable here, so we could make them as
# well be value types by switching them to be structs.
#
# ...
#
# Much better, about 400ms. We also have some other classes that could perfectly be structs,
# like Ray, Sphere, Light and Scene, so let's change them too.
#
# ...
#
# Now it's about 200ms, 10 times faster than the original. Of course you can't just turn
# everything into a struct, but it's good to know that you have that option in case
# you want to avoid some memory allocation.
